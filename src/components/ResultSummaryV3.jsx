...
function isCorrect(quiz, answer) {
  ...
  if (quiz.question_type === 'short_answer') {
   ...
   if (quiz.question_type === 'short_answer') {
    const correctAnswers = String(quiz.correct_answer_text || '')
      .split('//') // Tách các đáp án đúng
      .map(normalizeAnswer); // Chuẩn hóa từng đáp án
...
export default function ResultSummaryV3({ timeSpent, onRetake, quizzes, answers, topic }) {
